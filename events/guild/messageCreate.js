            ...
            if (cmd.length === 0) {
                if (mPrefix.includes(client.user.id))
                    return message.reply({
                        embeds: [new MessageEmbed()
                            .setColor(ee.color)
                            .setDescription(`✅ I'm Freak
My Prefix For This Server is ** \`${prefix}help\`**
Having Problems In Bot Join -   [Support Server](https://dsc.gg/freakbotsupport)`)
                        ]
                    }).catch(() => {});
                return;
            }
...
Having Problems In Bot Join  -   [Support Server](https://dsc.gg/freakbotsupport)`)
                    ...
                    commandLogsChannel.send({
                        embeds: [new MessageEmbed()
                            ...
                            .setTitle(`<a:ag_ARROW:959084917071937607> Prefix Command`)
                            .addField("**<a:ag_ARROW:959084917071937607> Author**", `\`\`\`${message.author.tag}\`\`\``)
                            .addField("**<a:ag_ARROW:959084917071937607> Command Name**", `\`\`\`${command.name}\`\`\``)
                        ]
                    });
                } catch (error) {
                    console.log(error)
